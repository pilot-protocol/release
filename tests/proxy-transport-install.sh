...
#   - proxy credentials never reach the installer output.
#
...
SH
cp "$FIXTURE/new/archive/pilotctl" "$FIXTURE/old/archive/pilotctl"
...
SANDBOX_CMD='bash -c '\''printf %s "${https_proxy:-$HTTPS_PROXY}"'\'''
...
if [ ! -d /run/systemd/system ]; then
    ...
    # A daemon without -proxy-cmd gets a note instead.
    LOG="$WORK/oldcmd.log"
    (export HTTPS_PROXY="$PROXY"
     run_install "$WORK/h-oldcmd" "$FIXTURE/old" "$LOG") || fail "old daemon sandbox install"
    [ -z "$(cfg_get "$WORK/h-oldcmd" proxy_cmd)" ] || fail "proxy_cmd saved for a daemon without -proxy-cmd"
